...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):           
        ...
        if(auth_principal.teacher_id is not None):
            assertions.assert_valid(assignment.teacher_id == auth_principal.teacher_id, 'the assignment was submitted to another teacher')  
            assertions.assert_valid(assignment.state != AssignmentStateEnum.GRADED, 'Permission denied for regrading')
    ...
    def get_assignments_by_teacher(cls, teacher_id):   
        return cls.filter(cls.teacher_id == teacher_id).all()  #fixed to fetch assignments by teacher id
